export default function Education(){
    return(
        <section className="py-20 px-2">
            <h2 className="text-6xl font-bold text-center mb-12">Education</h2>
            <div className="bg-zinc-800 p-6 rounded-lg border border-purple-500 shadow-lg shadow-purple-900/50">
                <h3 className="text-4xl font-bold mb-2">Bachelor of Computer Applications</h3>
                <p className="text-lg mb-4">Manav Rachna International Institute of Research and Technology, 2024-2027</p>
                <p className="text-gray-300">Dean's List — Top 5% of students</p>
                <p className="text-gray-300">Relevant coursework: DSA, DBMS, OOP (Java), Python, Web Development</p>
            </div>
        </section>      
    )
} 